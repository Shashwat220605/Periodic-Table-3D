...
function AtomicField({ intensity }) {
  ...
  return <mesh ref={ref}>
    ...
    <meshBasicMaterial color="#38bdf8" transparent opacity={0.035} depthWrite={false} wireframe />
  </mesh>;
}

function EnergyWave({ radius, speed, color = "#38bdf8" }) {
  const ref = useRef();
  useFrame(({ clock }) => {
    if (!ref.current) return;
    const phase = (clock.elapsedTime * speed) % 1;
    const scale = radius * (0.82 + phase * 0.28);
    ref.current.scale.set(scale, scale, scale);
    ref.current.material.opacity = (1 - phase) * 0.16;
  });
  return <mesh ref={ref}>
...
export default function Atom({ element, onInspect }) {
  ...
  const legendStyle = { width: "min(420px, 72vw)", padding: "10px 13px", border: "1px solid rgba(148,163,184,.16)", borderRadius: "12px", background: "rgba(2,6,23,.86)", backdropFilter: "blur(14px)", boxShadow: "0 12px 35px rgba(0,0,0,.35)", color: "#cbd5e1", fontFamily: "Inter, system-ui, sans-serif", pointerEvents: "auto" };

  return <group>
    <AtomicField intensity={energyMultiplier} />
    <EnergyWave radius={1.45} speed={0.45 * energyMultiplier} />
    <EnergyWave radius={2.5} speed={0.32 * energyMultiplier} color="#a78bfa" />
    ...
    <Html position={[0, 3.45, 0]} center distanceFactor={8} zIndexRange={[10, 20]}>
      <div style={legendStyle} onPointerDown={(event) => event.stopPropagation()}>
        <div style={{ fontSize: "7px", fontWeight: 800, letterSpacing: ".18em", color: "#64748b", marginBottom: "8px" }}>LIVE ATOMIC MODEL · PARTICLE KEY</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><span style={{ width: 9, height: 9, borderRadius: "50%", background: "#ef4444", boxShadow: "0 0 10px #ef4444" }} /><span><b style={{ display: "block", fontSize: "7px", color: "#fca5a5" }}>PROTON</b><small style={{ display: "block", fontSize: "5px", color: "#64748b" }}>+1 · NUCLEUS</small></span></div>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><span style={{ width: 9, height: 9, borderRadius: "50%", background: "#64748b", boxShadow: "0 0 8px rgba(100,116,139,.7)" }} /><span><b style={{ display: "block", fontSize: "7px", color: "#cbd5e1" }}>NEUTRON</b><small style={{ display: "block", fontSize: "5px", color: "#64748b" }}>0 · NUCLEUS</small></span></div>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><span style={{ width: 9, height: 9, borderRadius: "50%", background: "#38bdf8", boxShadow: "0 0 12px #38bdf8" }} /><span><b style={{ display: "block", fontSize: "7px", color: "#7dd3fc" }}>ELECTRON</b><small style={{ display: "block", fontSize: "5px", color: "#64748b" }}>−1 · ORBITAL</small></span></div>
        </div>
      </div>
    </Html>
