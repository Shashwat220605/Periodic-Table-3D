...
function acceptPoint(l,theta,phi,orientation){
  ...
  if(l===2){
    if(orientation===0) return Math.pow(Math.sin(theta)*Math.sin(2*phi),2);
    if(orientation===1) return Math.pow(Math.sin(theta)*Math.cos(2*phi),2);
    if(orientation===2) return Math.pow((3*y*y-1),2);
